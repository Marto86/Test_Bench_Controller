...
usb_device_desc_t device_desc_public = { .val = {0, 0, 0, 0, 0, 0, 0}};

usb_device_desc_t *get_device_desc_public()
{
    return &device_desc_public;
}
...
static void _cdc_data_task(void *arg)
{
    ...
    usb_device_desc_t *ddesc = get_device_desc_public();
    ESP_LOGI(TAG, "After get_device_desc_public()");
    if (ddesc != NULL)
    {
        ESP_LOGI(TAG, "Vendor and product VID: %x, PID: %x", ddesc->idVendor, ddesc->idProduct);


        if (ddesc->idVendor == EG915N_EU_VID && ddesc->idProduct == EG915N_EU_PID)
        {
            task_args->bulk_in_ep_desc.bEndpointAddress = EG915N_EU_IN_ADDR;
            task_args->bulk_out_ep_desc.bEndpointAddress = EG915N_EU_OUT_ADDR;
        }
        else if (ddesc->idVendor == E3372H_153_STICK_VID && ddesc->idProduct == E3372H_153_STICK_PID)
        {
            task_args->bulk_in_ep_desc.bEndpointAddress = E3372H_153_STICK_IN_ADDR;
            task_args->bulk_out_ep_desc.bEndpointAddress = E3372H_153_STICK_OUT_ADDR;
        }

        ESP_LOGI(TAG, "task_args->bulk_in_ep_desc.bEndpointAddress: %x", task_args->bulk_in_ep_desc.bEndpointAddress);
        ESP_LOGI(TAG, "task_args->bulk_out_ep_desc.bEndpointAddress: %x", task_args->bulk_out_ep_desc.bEndpointAddress);
    }
...
#ifdef CONFIG_CDC_GET_DEVICE_DESC
                    ret = _usb_get_dev_desc(pipe_hdl_dflt, &device_desc_public);
